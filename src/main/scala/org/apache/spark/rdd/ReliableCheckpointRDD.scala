...
private[spark] object ReliableCheckpointRDD extends Logging {
  ...
  def writePartitionToCheckpointFile[T: ClassTag](
    ...
    //sccomz Checkpoint Disable Check
    val checkpointDisable = env.conf.getBoolean("spark.streaming.checkpoint.disable", false)
    if(!checkpointDisable) {
  ...
  private def writePartitionerToCheckpointDir(
    ...
    try {
      ...
      val serializeStream = serializer.serializeStream(fileOutputStream)
      //sccomz Checkpoint Disable Check
      val checkpointDisable = sc.conf.getBoolean("spark.streaming.checkpoint.disable", false)
      if(!checkpointDisable) {
        Utils.tryWithSafeFinally {
